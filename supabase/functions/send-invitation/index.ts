...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Generate registration link (APP_URL secret, falls gesetzt)
    const appUrl = Deno.env.get('APP_URL') || 'https://ruffmichael.lovable.app';
    const registrationLink = `${appUrl}/auth`;

    // Compose SMS message
    const smsText = `Willkommen bei Ruff Michael! Bitte registriere dich in unserer Mitarbeiter-App:\n${registrationLink}`;
